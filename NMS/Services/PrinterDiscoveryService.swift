...
/// `WiFiSSIDService` are: this shells out and blocks on `waitUntilExit()`,
/// so it must be callable from a background queue. Without this, the
/// project-wide `SWIFT_DEFAULT_ACTOR_ISOLATION = MainActor` would confine
/// it to the main actor — which is exactly the bug this annotation was
/// added to fix (see `ConnectivityViewModel.refreshConfiguredPrinters`).
nonisolated struct PrinterDiscoveryService {
    ...
    // Real-time fault detection (out of paper, cover open, toner low) was
    // built here and removed — see DESIGN-NOTES.md's "Printer fault
    // detection (out of paper, cover open): a real dead end, on this
    // hardware" for the full story. In short: `lpstat -l -p`'s `Alerts:`
    // line and the standard SNMP Printer MIB (`prtAlertTable`/
    // `prtCoverStatus`) were both tried against a real fault (a
    // physically open drawer) and both came back empty or meaningless —
    // this Brother NC-8300h's firmware doesn't expose real-time fault
    // detail through either path while idle. A future contributor with
    // different printer hardware: check `lpstat -l -p`'s `Alerts:` line
    // and `snmpwalk -v2c -c public <printer-ip> 1.3.6.1.2.1.43.18.1.1`
    // (`prtAlertTable`) against a real fault condition first, the same
    // concrete-verification-before-code approach DESIGN-NOTES.md's entry
    // used — this printer's own dead end doesn't mean every printer's is.
}
