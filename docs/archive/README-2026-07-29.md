...
- **Wi-Fi network name (SSID)**: `WiFiSSIDService` reads the SSID via
  ...
  *physical* interface changes (Ethernet ↔ Wi-Fi), and roaming SSID-to-SSID
  ...
  no event at all. Confirmed against a real session switching between two
  named Wi-Fi networks (which changed subnet, router *and* DNS server): the
  only events were a generic interfaceDown/interfaceUp pair that named
  neither network, and no event anywhere in the run mentioned either SSID.
  ...
  takes over: home network → Ethernet → home network is correctly silent,
  while home network → Ethernet → guest network is still reported. Verified against the
