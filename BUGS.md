...
### "Stamp build info" writes nothing on the MacBook — footer shows "Build unknown"

- **Status**: Open, root-caused to "runs fine standalone, produces
  nothing under xcodebuild," true cause not yet isolated
- **Severity**: Medium — the build-hash label this session's own fix
  (`4104b24`) exists specifically to make trustworthy is silently absent
  on this machine, on every build; not a crash or data loss, but it
  defeats the exact stale-binary detection that caught the persistent
  store bug two entries below.
...
Pulled `21797a4` (includes `4104b24`, the "stamp build hash at build
time instead of reading git at runtime" fix, and the accompanying
`ENABLE_USER_SCRIPT_SANDBOXING = NO` on the NMS app target the iMac set
specifically because the sandboxed script phase can't read `.git`).
`script/build-and-run.sh` itself caught the regression: `warning: built
app has no NMSGitHash stamp -- BuildInfoService will report "unknown"`.

Confirmed the setting took effect — `xcodebuild -showBuildSettings`
reports `ENABLE_USER_SCRIPT_SANDBOXING = NO` for this target/config on
this machine, same as the iMac. Extracted the exact generated script
(`Script-1DDFCFAA3011D12A0069C74C.sh`) from DerivedData and ran it
directly with the real build's env vars (`SRCROOT`, `BUILT_PRODUCTS_DIR`,
`INFOPLIST_PATH`) — it worked perfectly, `git rev-parse`/`log`/`status`
all succeeded, all three plist keys written. Rebuilding through
`xcodebuild` again immediately after wipes those keys again (`Info.plist`
is regenerated by `ProcessInfoPlistFile` each build) and the phase
genuinely re-runs (`PhaseScriptExecution` appears in the log every time,
matching `alwaysOutOfDate = 1`) — but produces no keys.

So the script is correct and the flag is set correctly, yet the same
script invoked by xcodebuild itself behaves differently than the
identical command run by hand. Couldn't go further: checking the
unified log for an actual sandbox denial (`log show --predicate
'eventMessage contains "Sandbox"'`) requires Full Disk Access this
Terminal doesn't have, same class of restriction hit earlier this
session with `tccutil`. One difference worth checking machine-to-machine:
this Xcode's build uses the newer Swift Build system (`SWBBuildService`
visible in the script phase's own `$PATH`) — worth confirming the iMac's
build is using the same build system, since `ENABLE_USER_SCRIPT_SANDBOXING`
predates it and the two systems may not enforce it identically.
